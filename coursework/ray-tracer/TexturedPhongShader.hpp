...
private:
	...
	bool shadowTest_;
public:
	TexturedPhongShader(const std::vector<uint8_t>* albedoTexture, int texWidth, int texHeight, float shiniess, const Eigen::Vector3f& specular, bool shadowTest=true)
		:shadowTest_(shadowTest), albedoTexture_(albedoTexture),
		texWidth_(texWidth), texHeight_(texHeight),
		shininess_(shiniess), specular_(specular)
	{}
