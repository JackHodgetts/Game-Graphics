	...
	const Eigen::Vector3f& camWorldPos)
{
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			float depth0 = abs(t.cam[0].z()), depth1 = abs(t.cam[1].z()), depth2 = abs(t.cam[2].z());
			
			// Work out the depth at the point P
			float depthP = pow(((b0 / depth0) + (b1 / depth1) + (b2 / depth2)), -1);
			...
			// incorrect depths.
			float depth = depthP * ((t.screen[0].z() * b0 / depth0) + (t.screen[1].z() * b1 / depth1) + (t.screen[2].z() * b2 / depth2));
			// *** END YOUR CODE ***
