	...
	uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255)
{
	...
	minY = std::min(p0.x(), std::min(p1.y(), p2.y()));
	...
	triangleArea = abs((edge1.cross(edge3) / 2));
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			float a0 = abs(edge1.cross(p - p0) / 2);
			float a1 = abs(edge2.cross(p - p1) / 2);
			float a2 = abs(edge3.cross(p - p0) / 2);
